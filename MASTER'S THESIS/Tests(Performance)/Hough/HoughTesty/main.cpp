...
int main()
{
    ...
    for (int count = 0; count <  MAXCNT; count++)
    {
        ...
        img3 = imread("/home/pi/Pictures/Testy/blurr/reptile_look_chameleon_73102_1920x1080.jpg");
        ...
        Canny(img1, tmp1, 150, 200, 3);
        Canny(img2, tmp2, 150, 200, 3);
        Canny(img3, tmp3, 150, 200, 3);
        ...
        {
            timer.start();
            HoughLines(tmp1, dest, 1, CV_PI/180, 65, 0, 0 );
            time1 += timer.elapsed();

            timer.start();
            HoughLines(tmp2, dest, 1, CV_PI/180, 65, 0, 0 );
            time2 += timer.elapsed();

            timer.start();
            HoughLines(tmp3, dest, 1, CV_PI/180, 65, 0, 0 );
            time3 += timer.elapsed();
        }
#else
        {
            cout << " gray" << endl;
            timer.start();
            HoughLinesP(tmp1, dest, 1, CV_PI/180, 180, 0, 10 );
            time1 += timer.elapsed();

            timer.start();
            HoughLinesP(tmp2, dest, 1, CV_PI/180, 180, 0, 10 );
            time2 += timer.elapsed();

            timer.start();
            HoughLinesP(tmp3, dest, 1, CV_PI/180, 180, 0, 10 );
            time3 += timer.elapsed();
        }
#endif
    }
