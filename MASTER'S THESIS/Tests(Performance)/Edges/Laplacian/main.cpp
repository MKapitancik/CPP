...
int main()
{
    ...
    for (int count = 0; count <  MAXCNT; count++)
    {
        ...
        timer.start();
        Laplacian( tmp1, dst, ddepth, kernel_size, scale, delta, BORDER_DEFAULT );
        convertScaleAbs( dst, dest );
        time1 += timer.elapsed();

        timer.start();
        Laplacian( tmp2, dst, ddepth, kernel_size, scale, delta, BORDER_DEFAULT );
        convertScaleAbs( dst, dest );
        time2 += timer.elapsed();

        timer.start();
        Laplacian( tmp3, dst, ddepth, kernel_size, scale, delta, BORDER_DEFAULT );
        convertScaleAbs( dst, dest );
        time3 += timer.elapsed();
        dest = dest > 128;
    }
